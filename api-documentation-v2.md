...
**Version:** 2.0
**Database:** CortIQ
**Status:** Updated with approved Eligibility API design

---

## Introduction

The USA Volleyball API provides programmatic access to volleyball organization data including clubs, teams, member profiles, memberships, and eligibility verification. This API enables partners to integrate USA Volleyball data into their applications and services.
...
It evaluates eligibility at query time by checking a defined set of conditions against live data in CortIQ. It does not store eligibility results. It computes them fresh on every request.
...
|-----------|------|----------|---------|-------------|
| `role` | string | Yes | - | The role being checked (e.g., `player`, `coach`, `referee`). Required because eligibility rules differ by role. |
| `asOfDate` | date | No | Today's date | The date to evaluate against. Supports past dates (for audits) and future dates (for pre-registration). Format: `YYYY-MM-DD`. |
...
When a member is not eligible, the response indicates which conditions failed:
        ...
        "credentials": [
          {
            ...
            "reason": "Required credential incomplete: SafeSport Training"
          }
        ]
      }
    }
...
| `role` | string | The role that was evaluated |
| `asOfDate` | date | The date eligibility was evaluated against |
| `eligible` | boolean | Whether the member is eligible for the specified role on the specified date |
| `earliestExpiry` | date (nullable) | The soonest any passing condition expires, as an early warning signal. Null if no conditions have expiry dates. |
| `conditions` | array | Breakdown of each eligibility condition with pass/fail status |
| `conditions[].condition` | string | Description of the condition |
| `conditions[].passed` | boolean | Whether this condition passed |
| `conditions[].details` | object (nullable) | Additional details including expiry dates and failure reasons |

### List Valid Roles

A lightweight reference endpoint that lists valid role codes for use with the eligibility check. This response can be cached heavily.
...
The following conditions must all pass for a member to be eligible in a given role. These are evaluated in order for every request.

| # | Condition | Description |
|---|-----------|-------------|
| 1 | Role status is Active | The membership role must have an Active status |
| 2 | Membership has started | startDate <= evaluation date |
| 3 | Membership is not expired | endDate >= evaluation date |
...
| 6 | No active role-specific suspensions | No suspensions targeting the specific role being checked |
| 7 | All required credentials completed and valid | All credentials assigned to the role must have a status of Completed and not be expired |
...
**Fail closed.** If any data source is unavailable (e.g., the background check system is down), the answer is "ineligible" rather than returning a partial or optimistic result. Safety first.

**No caching of eligibility results.** Because eligibility is computed from live data, caching creates a window where a suspended person could appear eligible. If performance becomes an issue, the approach is to optimize the queries in CortIQ rather than caching the output.

**Condition sets are configurable, not hardcoded.** Each role has its own set of required conditions stored as configuration in CortIQ. This allows conditions to be updated without code changes when new requirements are added.

**Suspensions override everything.** Global suspensions (affect all roles) and role-specific suspensions (affect only certain roles) both block eligibility. A global suspension makes a person ineligible for any role regardless of all other conditions.
...
| `code` | string | Short code identifier for the club |
...
2. Membership must have started (startDate <= current date)
3. Membership must not be expired (endDate >= current date)
4. Membership status must be `Paid`
5. No active global suspensions on the profile
6. No active role-specific suspensions
7. All required credentials must be completed and valid

**Note:** Club affiliation is not an eligibility condition. Club assignments exist for organizational purposes but do not gate eligibility.

For real-time eligibility checks, use the Eligibility API endpoint (`GET /api/v1/eligibility/{memberId}`) documented above, which evaluates these conditions against live data and returns a detailed condition breakdown.
...
2. **Memberships and Profiles**: One profile can have multiple memberships across different seasons
3. **Membership Roles and Clubs**: Roles can be assigned to clubs for organizational purposes, but club assignment does not affect eligibility
...
To verify if a member is eligible to participate, use the Eligibility API:

```http
GET /api/v1/eligibility/{memberId}?role=player&asOfDate=2026-03-25
```

The response includes a simple `eligible` boolean along with a full condition breakdown. This evaluates all seven conditions in a single request against live data.

For pre-registration eligibility checks, pass a future `asOfDate`. For audit purposes, pass a past date.
...
3. Optionally use the Eligibility API to verify current eligibility for each team member
...
**Note:** Club transfers do not affect eligibility. A member remains eligible during a transfer as long as all other eligibility conditions are met.

---

## Open Questions

These items from the approved Eligibility API design recommendation still need answers before the eligibility endpoint can be finalized:

| ID | Question | Impact |
|----|----------|--------|
| OQ-01 | What is the full list of roles and their specific eligibility requirements? | Needs USAV national staff input. The API design can proceed, but per-role condition configuration depends on this. |
| OQ-02 | Do any credential types (e.g., referee certification) constitute a hard eligibility requirement? | The current design already treats credentials assigned to a role as blocking conditions. Should be confirmed with the team. |
| OQ-03 | Do unsigned waivers block eligibility? | Waivers are present in the data model but are not currently included as an eligibility condition. If confirmed as blocking, they can be added as a condition. |
